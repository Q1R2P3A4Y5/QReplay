...
def define_D(input_nc, ndf, netD,
    ...
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
    # print("[define_D] init_type:", init_type, type(init_type))
...
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64,
                 norm_layer=nn.BatchNorm2d, use_dropout=False, use_attention=True):
        ...
        unet_block = UnetSkipConnectionBlock(output_nc, ngf, input_nc=input_nc, submodule=unet_block, outermost=True, norm_layer=norm_layer, use_attention=use_attention)
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False, use_attention=True):
        ...
        if input_nc is None:
            input_nc = outer_nc
        downconv = DepthwiseSeparableConv(input_nc, inner_nc, kernel_size=4,
                             stride=2, padding=1, bias=use_bias)
        ...
        if outermost:
            ...
            upconv = nn.Sequential(
                DepthwiseSeparableConv(inner_nc * 2, outer_nc * 4, kernel_size=3, stride=1, padding=1, bias=use_bias),
                nn.PixelShuffle(upscale_factor=2)
            )

            down = [downrelu, downconv, downnorm, cbam]
...
class QRDiscriminatorWithDecoder(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        super(QRDiscriminatorWithDecoder, self).__init__()
      
        self.qr_detector = WeChatQRCode(
            detector_prototxt_path="detect.prototxt",
    ...
    def forward(self, x):
        ...
        for img in x.cpu().numpy():
            ...
            decoded_text, points = self.qr_detector.detectAndDecode(img)
            if decoded_text: 
                decoded_results.append(1.0)
            else:
                decoded_results.append(0.0)
